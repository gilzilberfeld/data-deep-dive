...
test.describe('Atomic Search and Discovery Tests', () => {
  ...
  test('should display a newly created book on the second page of the book list (#15)', async ({ page }) => {
    ...
    for (let i = 0; i < 11; i++) {
      const book = new BookBuilder().withTitle(`Book ${i}`).build();
      await factory.createBook(book);
    }
    const newBook = new BookBuilder().withTitle('The Twelfth Book').build();
    const targetBook = await factory.createBook(newBook);
